        ...
        if len(cluster_1_indices) > 1:
            ...
            for idx in cluster_1_indices:
                if idx != largest_area_idx:
                    original_width, original_height = all_cells[idx][2], all_cells[idx][3]
                    largest_width, largest_height = all_cells[largest_area_idx][2], all_cells[largest_area_idx][3]
                    if original_width < largest_width / 2 and original_height < largest_height / 2:
                        labels[idx] = 2  # move to cluster 2
                    else:
                        labels[idx] = -1  # remove from cluster 1
        start_idx += len(cells)
...
def plot_clusters(cells, labels):
    ...
    for label in unique_labels:
        if label == -1:  # skip removed elements
            continue
...
def process_new_image(image_path):
    ...
    # Дополнительная группировка для кластера 1
...
# Пример обработки нового изображения
new_image_path = 'unsupervised_learning/test_images/20.jpeg'
